...
static void Render(Scene *pScene, const std::string &Filename)
{
	...
	BlockGenerator BlockGenerator(OutputSize, RAIN_BLOCK_SIZE);
	...
	std::thread RenderThread([&]
		...
		auto Map = [&](const tbb::blocked_range<int> & Range)
		{
			...
			ImageBlock Block(Vector2i(RAIN_BLOCK_SIZE), pCamera->GetReconstructionFilter());
...
int main(int argc, char **argv)
{
	...
	try
	{
		if (Path.extension() == "xml")
		{
			...
			Rain::GetFileResolver()->prepend(Path.parent_path());

			std::unique_ptr<Rain::Object> Root(Rain::LoadFromXML(argv[1]));

			/* When the XML root object is a scene, start rendering it .. */
			if (Root->GetClassType() == Rain::Object::EClassType::EScene)
			{
				Rain::Render((Rain::Scene *)(Root.get()), argv[1]);
			}
		}
		else if (Path.extension() == "exr")
		{
			/* Alternatively, provide a basic OpenEXR image viewer */
			Rain::Bitmap Bit(argv[1]);
			Rain::ImageBlock Block(Rain::Vector2i(int(Bit.cols()), int(Bit.rows())), nullptr);
			Block.FromBitmap(Bit);
			std::unique_ptr<Rain::Screen> pScreen(new Rain::Screen(Block));
			pScreen->Draw();
		}
		else
		{
	...
	Rain::ObjectFactory::ReleaseAllocatedMemory();
