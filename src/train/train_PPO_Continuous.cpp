...
#include <sstream>
#include "RL/tool/getopt.hpp"
#include "RL/tool/utility.hpp"
#include "RL/tool/ring_vector.hpp"
#include "RL/env/gym_cpp/gymcpp.hpp"
#include "RL/network/dynet_network.hpp"
#include "RL/agent/ppo/PPO_Continuous_agent.hpp"
...
{
    auto total_file_name = ct::get_files_by_ext(basename, "*.params");
    ...
    for (int i_episode = 0; i_episode < n_episode; i_episode++) {
        ...
        if (i_episode >= 2000 && i_episode % 100 == 0) {
            ...
            if (test_rewards > best_reward) {
                ...
                agent.save_model(build_reward_model_name(model_name, test_rewards));
...
int main(int argc, char** argv)
{
    ...
    try {
        auto best_fnm = load_best_reward_model_name(model_name.str());
