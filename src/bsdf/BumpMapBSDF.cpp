...
void BumpMapBSDF::AddChild(Object * pChildObj, const std::string & Name)
{
	if (pChildObj->GetClassType() == EClassType::EBSDF)
	{
		...
		{
			throw RainException("BumpMapBSDF: tried to specify multiple nested BSDF");
		}
	}
	else if (pChildObj->GetClassType() == EClassType::ETexture)
	{
		...
		{
			throw RainException("BumpMapBSDF: tried to specify multiple bump map texture.");
		}
	}
}
...
void BumpMapBSDF::Activate()
{
	if (m_pBumpMap == nullptr)
	{
		throw RainException("BumpMapBSDF needs a nested BSDF!");
	}

	if (m_pNestedBSDF == nullptr)
	{
		throw RainException("BumpMapBSDF needs a bump map texture!");
	}
