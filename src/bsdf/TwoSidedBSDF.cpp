...
void TwoSidedBSDF::AddChild(Object * pChildObj, const std::string & Name)
{
	if (pChildObj->GetClassType() == EClassType::EBSDF)
	{
		...
		{
			throw RainException("TwoSidedBSDF: tried to specify the third nested BSDF");
		}
	}
}
...
void TwoSidedBSDF::Activate()
{
	if (m_pNestedBSDFs[0] == nullptr)
	{
		throw RainException("TwoSidedBSDF needs at least one nested BSDF!");
	}
	...
	{
		throw RainException("Only BSDF without transmission can be nested!");
	}
}
