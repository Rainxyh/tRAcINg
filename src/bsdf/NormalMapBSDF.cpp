...
void NormalMapBSDF::AddChild(Object * pChildObj, const std::string & Name)
{
	if (pChildObj->GetClassType() == EClassType::EBSDF)
	{
		...
		{
			throw RainException("NormalMapBSDF: tried to specify multiple nested BSDF");
		}
	}
	else if (pChildObj->GetClassType() == EClassType::ETexture)
	{
		...
		{
			throw RainException("NormalMapBSDF: tried to specify multiple normal map texture.");
		}
	}
}
...
void NormalMapBSDF::Activate()
{
	if (m_pNormalMap == nullptr)
	{
		throw RainException("NormalMapBSDF needs a nested BSDF!");
	}

	if (m_pNestedBSDF == nullptr)
	{
		throw RainException("NormalMapBSDF needs a bump map texture!");
	}
