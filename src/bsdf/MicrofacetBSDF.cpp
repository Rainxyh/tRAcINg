...
MicrofacetBSDF::MicrofacetBSDF(const PropertyList & PropList)
{
	...
	m_pAlpha = new ConstantFloatTexture(Clamp(PropList.GetFloat(XML_BSDF_MICROFACET_ALPHA, DEFAULT_BSDF_MICROFACET_ALPHA), float(MIN_ALPHA), float(MAX_ALPHA)));
...
void MicrofacetBSDF::AddChild(Object * pChildObj, const std::string & Name)
{
	if (pChildObj->GetClassType() == EClassType::ETexture && Name == XML_BSDF_MICROFACET_KD)
	{
		...
		{
			throw RainException("MicrofacetBSDF: tried to specify multiple kd texture");
		}
	}
	else if (pChildObj->GetClassType() == EClassType::ETexture && Name == XML_BSDF_MICROFACET_ALPHA)
	{
		...
		{
			throw RainException("MicrofacetBSDF: tried to specify multiple alpha texture");
		}
	}
	else
	{
		throw RainException("MicrofacetBSDF::AddChild(<%s>, <%s>) is not supported!",
