...
void StudentTTest::Activate()
{
	...
	if (!m_pBSDFs.empty())
	{
		...
		{
			throw RainException("Specified a different number of angles and reference values!");
		}
		if (!m_pScenes.empty())
		{
			throw RainException("Cannot test BSDFs and scenes at the same time!");
		}
	...
	else
	{
		...
		{
			throw RainException("Specified a different number of scenes and reference values!");
		}
