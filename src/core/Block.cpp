	...
	m_Offset(0, 0), m_Size(Size)
{
	if (pFilter != nullptr)
	{
		...
		m_pFilter.reset(new float[RAIN_FILTER_RESOLUTION + 1]);
		for (int i = 0; i < RAIN_FILTER_RESOLUTION; i++)
		{
			float Pos = (m_FilterRadius * i) / RAIN_FILTER_RESOLUTION;
			m_pFilter[i] = pFilter->Eval(Pos);
		}
		m_pFilter[RAIN_FILTER_RESOLUTION] = 0.0f;
		m_LookupFactor = RAIN_FILTER_RESOLUTION / m_FilterRadius;
...
void ImageBlock::FromBitmap(const Bitmap & Bitmap)
{
	...
	{
		throw RainException("Invalid bitmap dimensions!");
	}
