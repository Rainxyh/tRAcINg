...
bool ToBool(const std::string & String)
{
	...
	{
		throw RainException("Could not parse boolean value \"%s\"", String);
	}
}
...
int ToInt(const std::string & String)
{
	...
	if (*pEnd != '\0')
	{
		throw RainException("Could not parse integer value \"%s\"", String);
	}
	return Result;
}
...
float ToFloat(const std::string & String)
{
	...
	if (*pEnd != '\0')
	{
		throw RainException("Could not parse floating point value \"%s\"", String);
	}
	return Result;
}
...
Eigen::Vector3f ToVector3f(const std::string & String)
{
	...
	if (Tokens.size() != 3)
	{
		throw RainException("Expected 3 values");
	}
