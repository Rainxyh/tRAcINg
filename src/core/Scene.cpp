...
void Scene::Activate()
{
	...
	{
		throw RainException("No integrator was specified!");
	}

	if (m_pCamera == nullptr)
	{
		throw RainException("No camera was specified!");
	}
...
void Scene::AddChild(Object * pChildObj, const std::string & Name)
{
	...
	case EClassType::EAcceleration:
		if (m_pAcceleration != nullptr)
		{
			throw RainException("There can only be one acceleration per scene!");
		}
	...
	case EClassType::EEmitter:
		...
		else if (((Emitter*)(pChildObj))->GetEmitterType() == EEmitterType::EEnvironment)
		{
			...
			{
				throw RainException("Scene::AddChild(): Only one environment emiiter is allowed for the entire scene");
			}
		}
		else
		{
			throw RainException("Scene::AddChild(): You need to implement this for emitters");
		}
		break;
	case EClassType::ESampler:
		if (m_pSampler != nullptr)
		{
			throw RainException("There can only be one sampler per scene!");
		}
	...
	case EClassType::ECamera:
		if (m_pCamera)
		{
			throw RainException("There can only be one camera per scene!");
		}
	...
	case EClassType::EIntegrator:
		if (m_pIntegrator)
		{
			throw RainException("There can only be one integrator per scene!");
		}
		m_pIntegrator = (Integrator*)(pChildObj);
		break;
	default:
		throw RainException("Scene::AddChild(<%s>, <%s>) is not supported!",
