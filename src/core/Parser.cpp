...
Object * LoadFromXML(const std::string & Filename)
{
	...
	{
		throw RainException("Error while parsing \"%s\": %s (at %s)", Filename, Result.description(), OffsetFunc(Result.offset));
	}
		...
		try
		{
			...
			else
			{
				...
				case ETag::EMatrix:
				{
					...
					if (Tokens.size() != 16)
					{
						throw RainException("Expected 16 values");
					}
					Eigen::Matrix4f Mat;
					for (int i = 0; i < 4; ++i)
					{
				...
				default: 
					throw RainException("Unhandled element \"%s\"", Node.name());
				};
			}
		}
		catch (const RainException & Ex) 
		{
			throw RainException(
