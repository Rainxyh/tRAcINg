...
BitmapTexture::BitmapTexture(const PropertyList & PropList)
{
	...
	if (WrapMode != "")
	{
		...
		else { throw RainException("Illegal wrap mode [%s].", WrapMode.c_str()); }
	}
	else
	{
		...
		else { throw RainException("Illegal wrap mode [%s].", UWrapMode.c_str()); }
		...
		else { throw RainException("Illegal wrap mode [%s].", VWrapMode.c_str()); }
	...
	else { throw RainException("Illegal filter type [%s].", FilterType.c_str()); }
	...
	else { throw RainException("Illegal channel [%s].", Channel.c_str()); }
