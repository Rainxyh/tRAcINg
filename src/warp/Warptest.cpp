...
using Rain::RainException;
using Rain::ObjectFactory;
using Rain::Point2f;
using Rain::Point2i;
using Rain::Point3f;
using Rain::Warp;
using Rain::PropertyList;
using Rain::BSDF;
using Rain::BSDFQueryRecord;
using Rain::Color3f;
...
struct WarpTest {
    ...
    WarpTest(WarpType warpType_, float parameterValue_, BSDF *bsdf_ = nullptr,
             BSDFQueryRecord bRec_ = BSDFQueryRecord(Rain::Vector3f()),
    ...
    std::pair<bool, std::string> run() {
        ...
        Rain::MatrixXf points, values;
        generatePoints(sampleCount, Independent, points, values);

        for (int i=0; i<sampleCount; ++i) {
            if (values(0, i) == 0)
                continue;
            Rain::Vector3f sample = points.col(i);
            float x, y;
        ...
        auto integrand = [&](double y, double x) -> double {
            if (warpType == Square) {
                ...
                Rain::Vector3f v((float) (sinTheta * cosPhi),
                ...
                if (warpType == UniformSphere)
                    ...
                    br.wo = v;
                    br.measure = Rain::ESolidAngle;
                    return bsdf->pdf(br);
                } else {
                    throw RainException("Invalid warp type");
                }
            }
        };
        ...
        for (int y=0; y<yres; ++y) {
            ...
            for (int x=0; x<xres; ++x) {
                ...
                if (ptr[y * xres + x] < 0)
                    throw RainException("The Pdf() function returned negative values!");
            }
        }
    ...
    void generatePoints(int &pointCount, PointType pointType,
                        Rain::MatrixXf &positions, Rain::MatrixXf &weights) {
    ...
    create_microfacet_bsdf(float alpha, float kd, float bsdfAngle) {
        ...
        Rain::Vector3f wi(std::sin(bsdfAngle), 0.f,
...
struct Arcball {
    ...
    Arcball(float speedFactor = 2.0f)
        : m_active(false), m_lastPos(Rain::Vector2i::Zero()), m_size(Rain::Vector2i::Zero()),
          m_quat(Quaternionf::Identity()),
          m_incr(Quaternionf::Identity()),
          m_speedFactor(speedFactor) { }

    void setSize(Rain::Vector2i size) { m_size = size; }

    const Rain::Vector2i &size() const { return m_size; }

    void button(Rain::Vector2i pos, bool pressed) {
        m_active = pressed;
        m_lastPos = pos;
        if (!m_active)
    ...
    bool motion(Rain::Vector2i pos) {
        ...
        Rain::Vector3f v0(ox, oy, 1.0f), v1(tx, ty, 1.0f);
        if (v0.squaredNorm() > 1e-4f && v1.squaredNorm() > 1e-4f) {
            v0.normalize(); v1.normalize();
            Rain::Vector3f axis = v0.cross(v1);
...
private:
    ...
    Rain::Vector2i m_lastPos;

    /// The size of this Arcball.
    Rain::Vector2i m_size;
...
public:
    ...
    WarpTestScreen(): Screen(Vector2i(1024, 768), "warptest: Sampling and Warping"), m_bRec(Rain::Vector3f()) {
        inc_ref();
    ...
    void refresh() {
        ...
        if (warpType != Square) {
            for (int i=0; i < m_pointCount; ++i) {
                if (values(0, i) == 0.0f) {
                    positions.col(i) = Rain::Vector3f::Constant(std::numeric_limits<float>::quiet_NaN());
                    continue;
                }
                positions.col(i) =
                    ((value_scale == 0 ? 1.0f : (value_scale * values(0, i))) *
                     positions.col(i)) * 0.5f + Rain::Vector3f(0.5f, 0.5f, 0.0f);
            }
        }

        /* Generate a color gradient */
        Rain::MatrixXf colors(3, m_pointCount);
        ...
        if (m_gridCheckBox->checked()) {
            ...
            if (warpType != Square) {
                for (int i=0; i<m_lineCount; ++i)
                    positions.col(i) = positions.col(i) * 0.5f + Rain::Vector3f(0.5f, 0.5f, 0.0f);
            }
    ...
    bool mouse_motion_event(const Vector2i &p, const Vector2i &rel,
        ...
        if (!Screen::mouse_motion_event(p, rel, button, modifiers))
            m_arcball.motion(Rain::Vector2i(p.x(), p.y()));
        return true;
    }
    ...
    bool mouse_button_event(const Vector2i &p, int button, bool down, int modifiers) {
        ...
        if (!Screen::mouse_button_event(p, button, down, modifiers)) {
            if (button == GLFW_MOUSE_BUTTON_1)
                m_arcball.button(Rain::Vector2i(p.x(), p.y()), down);
        }
        return true;
    }
    ...
    void initializeGUI() {
        ...
        set_resize_callback([&](Vector2i size) {
            m_arcball.setSize(Rain::Vector2i(size.x(), size.y()));
        });

        m_arcball.setSize(Rain::Vector2i(m_size.x(), m_size.y()));
...
int main(int argc, char **argv) {
    ...
    auto bRec = BSDFQueryRecord(Rain::Vector3f());
