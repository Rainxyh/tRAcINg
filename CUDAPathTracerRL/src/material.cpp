...
float xorshf96()
{ // 3.6 times better performance than rand()
...
__host__ __device__ Color Material::shade(const Ray &incident, const bool isSolid) const
{
	...
	if (kt > 0)
	{
		...
		else
		{
			...
			if (incident.getLevel() > 1)
				if (xorshf96() < P)
					return finalColor * world->shade_ray(reflectedRay) * RP;
				else
					return finalColor * world->shade_ray(refractedRay) * TP;
	...
	else if (!kr || xorshf96() > kr)
	{ // kr=1 means completely reflective
